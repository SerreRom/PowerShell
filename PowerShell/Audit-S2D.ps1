...
# Specify the path to HTML export file
$ExportLog   = "c:\temp\S2D-Audit.html"
...
<html> 
<Head>
<Title>Audit of $ClientName Hyper-V/S2D infrastructure - Date: $Date</title>
<Style>
body
{
...
$HTMLEnding = @"
<center><span class="AdvertMessage>This board has been generated by PowerShell script written by Romain Serre (@RomSerre)</span></center>
</body>
</html>
"@
...
if ($ClustNetInformation){
    ...
    Foreach ($Network in  $ClusterNetInformation){
        ...
        if ($Network.LMNet){
            # If cluster communication are not enabled, export CSS class error
            if ($Network.Role -ne 1){
                $LMClass = $TDError
            }
